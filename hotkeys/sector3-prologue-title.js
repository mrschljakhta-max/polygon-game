...
#vidlikPrologueTitle .vpt-bg{position:absolute;inset:-2%;z-index:0;background-image:url('data:image/webp;base64,${BG}');background-size:cover;background-position:center center;background-repeat:no-repeat;transform:scale(1.025);animation:vpt-drift 14s ease-out forwards}
...
let introReady=null;
function loadIntro(){
 if(introLoaded)return introReady||Promise.resolve();
 introLoaded=true;
 document.body.classList.remove('prologue-title-pending');
 introReady=new Promise(resolve=>{
   const s=document.createElement('script');
   s.src='sector3-intro.js?v=20260907-prologue2';
