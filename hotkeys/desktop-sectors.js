...
document.addEventListener('fullscreenchange',()=>{if(changingFullscreen||phase!=='play'||document.fullscreenElement)return;captureState={fullscreen:false,keyboardLock:false};leaveLevel()});
window.addEventListener('keydown',e=>{if(phase!=='play')return;if(e.code==='Escape'){e.preventDefault();leaveLevel();return}if(paused)return;if(e.code==='F1'&&level!==9){e.preventDefault();hints++;render();return}if(keyboard.shouldPrevent(e,true))e.preventDefault();if(e.repeat||locked)return;pressed.add(e.code);if(keyboard.isModifier(e.code)){render();return}const t=current();const ok=keyboard.matches(e,t);if(ok){locked=true;feedback='correct';render();clearTimeout(nextTimer);nextTimer=setTimeout(()=>{locked=false;pressed.clear();feedback='idle';if(task>=SECTORS[sector].levels[level].tasks.length-1){setCompleted(sector,level);phase='complete';stop();keyboard.release(false);render()}else{task++;render();focusTask()}},1500)}else{errors++;feedback='wrong';render();setTimeout(()=>{if(feedback==='wrong'&&!locked){feedback='idle';render()}},550)}},true);
