...
let phase='hub',sector=2,level=0,task=0,seconds=0,errors=0,hints=0,feedback='idle',pressed=new Set(),paused=false,locked=false,timer=null,nextTimer=null;
...
function bind(){document.querySelectorAll('[data-sector]').forEach(b=>b.addEventListener('click',()=>{const n=+b.dataset.sector;if(n===1){location.href='game.html';return}sector=n;phase='levels';render()}));document.getElementById('ds-sector1')?.addEventListener('click',()=>location.href='game.html');document.getElementById('ds-back-hub')?.addEventListener('click',()=>{phase='hub';render()});document.querySelectorAll('[data-level]').forEach(b=>b.addEventListener('click',()=>start(+b.dataset.level)));document.getElementById('ds-hint')?.addEventListener('click',()=>{hints++;render()});document.getElementById('ds-exit')?.addEventListener('click',()=>{stop();phase='levels';paused=false;render()});document.getElementById('ds-resume')?.addEventListener('click',()=>{paused=false;run();render();focusTask()});document.getElementById('ds-repeat-task')?.addEventListener('click',()=>{clearTimeout(nextTimer);feedback='idle';locked=false;pressed.clear();paused=false;run();render();focusTask()});document.getElementById('ds-restart')?.addEventListener('click',()=>start(level));document.getElementById('ds-levels')?.addEventListener('click',()=>{stop();paused=false;phase='levels';render()});document.getElementById('ds-next')?.addEventListener('click',()=>{if(level<9)start(level+1);else{phase='levels';render()}});document.getElementById('ds-repeat')?.addEventListener('click',()=>start(level))}
function start(i){if(!unlocked(sector,i))return;clearTimeout(nextTimer);level=i;task=0;seconds=errors=hints=0;feedback='idle';pressed.clear();paused=false;locked=false;phase='play';run();render()}
...
function modifierMatch(e,t){return (!!e.ctrlKey===!!t.ctrl)&& (!!e.shiftKey===!!t.shift)&& (!!e.altKey===!!t.alt)}
function shouldBlock(e){if(phase!=='play'||paused)return false;return e.ctrlKey||e.altKey||e.shiftKey||['F1','F2','Home','End','PageUp','PageDown','Space','Tab','Enter','Delete','Backspace','ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.code)}
window.addEventListener('keydown',e=>{if(phase!=='play')return;if(paused){if(e.code==='Escape'){e.preventDefault();paused=false;run();render();focusTask()}return}if(e.code==='Escape'){e.preventDefault();paused=true;stop();render();return}if(e.code==='F1'&&level!==9){e.preventDefault();hints++;render();return}if(shouldBlock(e))e.preventDefault();if(e.repeat||locked)return;pressed.add(e.code);if(['ControlLeft','ControlRight','ShiftLeft','ShiftRight','AltLeft','AltRight'].includes(e.code)){render();return}const t=current();const ok=e.code===t.code&&modifierMatch(e,t);if(ok){locked=true;feedback='correct';render();clearTimeout(nextTimer);nextTimer=setTimeout(()=>{locked=false;pressed.clear();feedback='idle';if(task>=SECTORS[sector].levels[level].tasks.length-1){setCompleted(sector,level);phase='complete';stop();render()}else{task++;render();focusTask()}},1500)}else{errors++;feedback='wrong';render();setTimeout(()=>{if(feedback==='wrong'&&!locked){feedback='idle';render()}},550)}},true);
