...
let phase='hub',sector=2,level=0,task=0,seconds=0,errors=0,hints=0,feedback='idle',pressed=new Set(),paused=false,locked=false,timer=null,nextTimer=null,captureState={fullscreen:false,keyboardLock:false},changingFullscreen=false,pauseGuardUntil=0;
...
function render(){if(phase==='hub')app.innerHTML=hub();else if(phase==='complete')app.innerHTML=result();else app.innerHTML=play();bind()}
function bind(){document.querySelectorAll('[data-sector]').forEach(b=>b.addEventListener('click',()=>{const n=+b.dataset.sector;location.href=`module-briefing.html?sector=${n}`}));document.getElementById('ds-sector1')?.addEventListener('click',()=>location.href='module-briefing.html?sector=1');document.getElementById('ds-hint')?.addEventListener('click',()=>{hints++;render()});document.getElementById('ds-exit')?.addEventListener('click',pauseLevel);document.getElementById('ds-resume')?.addEventListener('click',resumeLevel);document.getElementById('ds-repeat-task')?.addEventListener('click',()=>{clearTimeout(nextTimer);feedback='idle';locked=false;pressed.clear();resumeLevel()});document.getElementById('ds-restart')?.addEventListener('click',()=>start(level));document.getElementById('ds-briefing')?.addEventListener('click',leaveLevel);document.getElementById('ds-next')?.addEventListener('click',()=>{if(level<9)start(level+1);else leaveLevel()});document.getElementById('ds-repeat')?.addEventListener('click',()=>start(level))}
...
async function pauseLevel(){if(phase!=='play'||paused)return;clearTimeout(nextTimer);nextTimer=null;paused=true;pauseGuardUntil=performance.now()+350;locked=false;feedback='idle';pressed.clear();stop();render();changingFullscreen=true;await keyboard.release(true);captureState={fullscreen:false,keyboardLock:false};changingFullscreen=false}
async function leaveLevel(){stop();paused=false;phase='leaving';pressed.clear();locked=false;changingFullscreen=true;await keyboard.release(true);changingFullscreen=false;const fromBriefing=new URLSearchParams(location.search).get('from')==='briefing';if(fromBriefing&&history.length>1){history.back();return}location.replace(`module-briefing.html?sector=${sector}`)}
...
document.addEventListener('fullscreenchange',()=>{if(changingFullscreen||phase!=='play'||paused||document.fullscreenElement)return;captureState={fullscreen:false,keyboardLock:false};pauseLevel()});
window.addEventListener('keydown',e=>{if(phase!=='play')return;if(e.code==='Escape'){e.preventDefault();if(paused){if(performance.now()>=pauseGuardUntil)resumeLevel()}else pauseLevel();return}if(paused)return;if(!captureState.fullscreen&&!changingFullscreen){changingFullscreen=true;keyboard.capture(shell).then(state=>{captureState=state}).finally(()=>{changingFullscreen=false})}if(e.code==='F1'&&level!==9){e.preventDefault();hints++;render();return}if(keyboard.shouldPrevent(e,true))e.preventDefault();if(e.repeat||locked)return;pressed.add(e.code);if(keyboard.isModifier(e.code)){render();return}const t=current();const ok=keyboard.matches(e,t);if(ok){locked=true;feedback='correct';render();clearTimeout(nextTimer);nextTimer=setTimeout(()=>{locked=false;pressed.clear();feedback='idle';if(task>=SECTORS[sector].levels[level].tasks.length-1){setCompleted(sector,level);phase='complete';stop();keyboard.release(false);render()}else{task++;render();focusTask()}},1500)}else{errors++;feedback='wrong';render();setTimeout(()=>{if(feedback==='wrong'&&!locked){feedback='idle';render()}},550)}},true);
window.addEventListener('keyup',e=>{pressed.delete(e.code);if(phase==='play'&&!paused&&!locked)render()},true);
function boot(){const params=new URLSearchParams(location.search);const directSector=parseInt(params.get('sector')||params.get('directSector')||'',10);const directLevel=parseInt(params.get('level')||'',10);const direct=params.has('directSector')||(params.get('from')==='briefing'&&params.has('sector'));if(!direct){render();return}if(!(directSector>=2&&directSector<=8&&directLevel>=1&&directLevel<=10)){location.replace('desktop-sectors.html?v=15');return}sector=directSector;if(!unlocked(sector,directLevel-1)){location.replace(`module-briefing.html?sector=${sector}`);return}params.delete('directSector');params.set('sector',String(sector));history.replaceState(null,'',location.pathname+'?'+params.toString());start(directLevel-1)}
boot();
})();
